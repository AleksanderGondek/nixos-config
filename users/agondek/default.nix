{
  ...
  cfg = config.agondek-nixos-config.users.agondek;
  secrets = config.sops.secrets;
in {
  # TODO: Please split me into more modules :)
  imports = [];
  ...
  options.agondek-nixos-config.users.agondek = {
    ...
    desktopMode = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = ''
        If enabled, default agondek user will define packages / settings,
        which are useful only in the desktop mode (outside of console terminal).
      '';
    };
  };
